...
public class SearchController implements Initializable, BaseController {
    ...
    public void turnOnEnViMode() {
        if (mode == null || mode == Dictionary.MODE.VIETNAMESE) {
            lastWordSearch = null;
    ...
    public void turnOnViEnMode() {
        if (mode == null || mode == Dictionary.MODE.ENGLISH) {
            lastWordSearch = null;
    ...
    public void handleCancelEditButton(ActionEvent event) {
        ...
        closeEditAfterCancel();
    }

    public void closeEditAfterCancel() {
        submitEditButton.setVisible(false);
        cancelEditButton.setVisible(false);
    ...
    public void handleSubmitEditButton(ActionEvent event) {
        String wordData = dictionary.getWordData(lastWordSearch);
        String newWordData = explainField.getText();
        if (newWordData.isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setContentText("The translation can not be empty !");
            Optional<ButtonType> result = alert.showAndWait();
            return;
        }
